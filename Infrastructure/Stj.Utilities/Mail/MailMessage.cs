...
namespace Stj.Utilities.Mail
{
  ...
  internal static class MailMessageExtension
  {
    ...
    ///   RFC2156.
    /// </summary>
    private static readonly Dictionary<MailPriority, string> PriorityMap =
    ...
    ///   RFC2156.
    /// </summary>
    private static readonly Dictionary<MailPriority, string> ImportanceMap =
      new Dictionary<MailPriority, string>
      {
    ...
    /// </summary>
    /// <param name="message">
    ///   The MailMessage instance to construct the
    ///   textual representation from.
    /// </param>
    /// <returns>
    ///   An RFC822/MIME-compliant string describing a mail
    ///   message.
    /// </returns>
    /// <exception cref="InvalidOperationException">
    ///   Thrown if the From
    ///   property is null or is not properly initialized.
    /// </exception>
    ...
    /// </summary>
    /// <param name="m">
    ///   The MailMessage instance to build the header
    ///   from.
    /// </param>
    /// <returns>
    ///   A NameValueCollection representing the RFC822/MIME
    ///   mail header fields.
    /// </returns>
    private static NameValueCollection BuildHeader(MailMessage m)
    {
      string[] ignore =
      {
      ...
      {
        var contentType = (m.Attachments.Count == 0
    ...
    /// <returns>
    ///   The input string encoded as Q-encoded string containing
    ///   only ASCII characters.
    /// </returns>
    private static string QEncode(string s)
    {
      var builder = new StringBuilder("=?UTF-8?Q?");
      var chars = System.Text.Encoding.Unicode.GetChars(System.Text.Encoding.Unicode.GetBytes(s));
    ...
    /// </summary>
    /// <param name="address">
    ///   The MailAddress instance to create the address
    ///   string from.
    /// </param>
    /// <returns>An address string as is used in RFC822 mail headers</returns>
    private static string To822Address(this MailAddress address)
    {
      if (!string.IsNullOrEmpty(address.DisplayName))
      {
        var name = address.DisplayName.IsASCII()
          ? address.DisplayName
          : QEncode(address.DisplayName);
    ...
    /// </summary>
    /// <param name="m">
    ///   The MailMessage instance to build the mail body
    ///   from.
    /// </param>
    /// <param name="header">
    ///   The RFC822/MIME mail header to use for
    ...
    /// </returns>
    /// <remarks>
    ///   According to RFC2822 each line of a mail message should
    ...
    ///   line breaks are inserted to meet this requirement.
    /// </remarks>
    private static string BuildBody(MailMessage m, NameValueCollection header)
    {
      var builder = new StringBuilder();
    ...
    ///   constructing the mail body.
    /// </param>
    /// <param name="addHeaders">
    ///   Set to true to append body headers before
    ///   adding the actual body part content.
    /// </param>
    private static void AddBody(StringBuilder builder, MailMessage m,
    ...
    /// <param name="m">
    ///   The MailMessage instance whose AlternateView collection
    ///   will be added to the nested multipart/alternative part.
    /// </param>
    /// <param name="header">
    ///   The RFC822/MIME mail header to use for
    ///   constructing the mail body.
    /// </param>
    /// <remarks>
    ///   This is used if the MailMessage instance contains both alternative
    ///   views and attachments. In this case the created RFC822/MIME mail message will
    ///   contain nested body parts.
    /// </remarks>
    private static void AddNestedAlternative(StringBuilder builder, MailMessage m,
    ...
    /// <param name="builder">The StringBuilder instance to append to.</param>
    /// <param name="m">
    ///   The MailMessage instance whose Attachments collection
    ///   will be added to the nested multipart/mixed part.
    /// </param>
    /// <remarks>
    ///   This is used if the MailMessage instance contains both alternative
    ///   views and attachments. In this case the created RFC822/MIME mail message will
    ///   contain nested body parts.
    /// </remarks>
    private static void AddNestedMixed(StringBuilder builder, MailMessage m)
    {
      var boundary = GenerateContentBoundary();
