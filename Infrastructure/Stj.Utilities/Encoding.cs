...
namespace Stj.Utilities
{
    internal static class Encoding
    {
        ...
        internal static string DecodeWord(string word)
        {
            ...
            if (!m.Success)
                return word;
            System.Text.Encoding encoding = Encoding.GetEncoding(m.Groups[1].Value);
            string type = m.Groups[2].Value.ToUpper();
            string text = m.Groups[3].Value;
            switch (type)
            {
                case "Q":
                    return Encoding.QDecode(text, encoding);
                case "B":
                    return encoding.GetString(Encoding.Base64Decode(text));
                default:
        ...
        internal static string QDecode(string value, System.Text.Encoding encoding)
        {
            try
            {
        ...
        internal static string QPDecode(string value, System.Text.Encoding encoding)
        {
            try
            {
                using (MemoryStream m = new MemoryStream())
        ...
        /// be resolved.</returns>
        internal static System.Text.Encoding GetEncoding(string name)
        {
