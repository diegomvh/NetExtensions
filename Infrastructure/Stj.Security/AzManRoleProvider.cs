...
namespace Stj.Security
{
    public class AzManRoleProvider : RoleProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            {
                throw new AzManProviderException(SecurityResources.MessageAzManApplicationNameNotSpecified);
            }

            ConnectionString = Configuration.Settings.AzManConnectionString;
            if (string.IsNullOrEmpty(ConnectionString))
            {
                throw new AzManProviderException(SecurityResources.MessageAzManConnectionStringNotSpecified);
            }
        }  
        ...
        public override void AddUsersToRoles(string[] userNames, string[] roleNames)
        {
            ...
            foreach (var roleName in roleNames)
            {
                ...
                if (role == null)
                {
                    throw new ProviderException(string.Format(SecurityResources.MessageAzManRoleDoesNotExist, roleName));
                }
                roles[i++] = role;
            }
            ...
            using (var store = new AzManStore(ApplicationName, ConnectionString))
            {
                try
                {
                    foreach (var userName in userNames)
                    {
                        ...
                        if (user == null)
                        {
                            throw new ProviderException(string.Format(SecurityResources.MessageAzManUserDoesNotExist, userName));
                        }
                        ...
                        {
                            throw new AzManProviderException(SecurityResources.MessageAzManAnExceptionOccured, ex);
                        }
                    }
                }
                finally
        ...
        public override void CreateRole(string roleName)
        {
            ...
            using (var store = new AzManStore(ApplicationName, ConnectionString))
            {
                ...
                {
                    throw new ProviderException(string.Format(SecurityResources.MessageAzManRoleAlreadyExists, roleName));
                }
        ...
        public override bool DeleteRole(string roleName, bool throwOnPopulatedRole)
        {
            ...
            if (throwOnPopulatedRole)
            {
                ...
                {
                    throw new ProviderException(string.Format(SecurityResources.MessageAzManRoleIsNotEmpty, roleName));
                }
            }
        ...
        private void CheckParameter(ref string param, bool checkForNull, bool checkIfEmpty, bool checkForCommas, int maxSize, string paramName)
        {
            ...
            else
            {
                ...
                {
                    throw new ArgumentException(string.Format(SecurityResources.MessageAzManParameterIsEmpty, paramName));
                }
                if ((maxSize > 0) && (param.Length > maxSize))
                {
                    throw new ArgumentException(string.Format(SecurityResources.MessageAzManParameterTooLong, paramName, maxSize));
                }
                if (checkForCommas && param.Contains(","))
                {
                    throw new ArgumentException(string.Format(SecurityResources.MessageAzManParameterCannotContainCommas, paramName));
                }
        ...
        private void CheckArrayParameter(ref string[] param, bool checkForNull, bool checkIfEmpty, bool checkForCommas, int maxSize, string paramName)
        {
            ...
            if (param.Length < 1)
            {
                throw new ArgumentException(string.Format(SecurityResources.MessageAzManParameterArrayCannotBeEmpty, paramName));
            }
            ...
            for (int i = param.Length - 1; i >= 0; i--)
            {
                ...
                {
                    throw new ArgumentException(string.Format(SecurityResources.MessageAzManParameterArrayCannotContainDuplicates, paramName));
                }
