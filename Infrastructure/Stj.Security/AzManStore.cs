...
namespace Stj.Security
{
    internal class AzManStore : IDisposable
    {
        ...
        {
            if (string.IsNullOrEmpty(applicationName)) throw new AzManProviderException(SecurityResources.MessageAzManApplicationNameNotSpecified);
            ...
            {
                throw new AzManProviderException(SecurityResources.MessageAzManHelperInitializeFailed, ex);
            }
            catch (Exception ex)
            {
                throw new AzManProviderException(string.Format(SecurityResources.MessageAzManInvalidConnectionString, connectionString), ex);
            }
        }
